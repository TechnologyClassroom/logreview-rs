...
fn timeisthemaster() {
    let date = Local::now();
    println!("\nCurrent date/time: {}", date.format("[%Y-%m-%d][%H:%M:%S]"));
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("\nConfig file loaded successfully!");  // Debug

    // Display results
    display_results(config);

    // Set the log to scan.
    let filename = "log/test1.log"; // This works.
    //let filename = config.filenames; // This does not work.
